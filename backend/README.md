...
Configuration is environment-driven. Copy `../env.example` (or `backend/env.example`) to `../.env` and set:
...
Use the credentials from `ADMIN_SEED_EMAIL` / `ADMIN_SEED_PASSWORD`. If unset, defaults are `nijojapan@gmail.com` / `nijo@123` (override in production as needed).
